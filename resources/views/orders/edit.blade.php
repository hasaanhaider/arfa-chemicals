    ...
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        ...
        <div class="card">
            <form class="card-body" method="POST" action="{{ route('orders.store') }}">
                @if(session('success'))
                <div class="flex gap-1 px-4 py-3 text-sm text-green-500 border border-green-200 rounded-md md:items-center bg-green-50 dark:bg-green-400/20 dark:border-green-500/50">
                    <i data-lucide="alert-circle" class="h-4"></i> {{ session('success') }}
                </div>
                @elseif (session('error'))
                <div class="flex gap-1 px-4 py-3 text-sm text-red-500 border border-red-200 rounded-md md:items-center bg-red-50 dark:bg-red-400/20 dark:border-red-500/50">
                    <i data-lucide="alert-circle" class="h-4"></i> {{ session('error') }}
                </div>
                @endif
                @csrf
                <h6 class="mb-4 text-15">Add Order</h6>
                <div class="grid items-center grid-cols-1 gap-5 xl:grid-cols-3">
                    ...
                    <div>
                        ...
                        <input type="number" name="factory_id" id="factory_id"
                            class="px-5 py-3 text-15 form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500"
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            class="px-5 py-3 text-15 form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="price_excluding_tax" id="price_excluding_tax"
                            class="px-5 py-3 text-15 form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500"
